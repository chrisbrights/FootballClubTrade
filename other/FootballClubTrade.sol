...
function getVelocity(uint256 specificTime) external view returns (int256[] memory) {
    // Ensure the specific time is valid (i.e., it's in the past)
    require(specificTime < block.timestamp, "Specific time must be in the past");

    // Initialize an array to store velocities
    int256[] memory velocities = new int256[](clubs.length);

    // Loop through each club to retrieve the stored velocity
    for (uint256 i = 0; i < clubs.length; i++) {
        // Retrieve the velocity which should be previously set by `setVelocity`
        velocities[i] = clubs[i].velocity;
    }
